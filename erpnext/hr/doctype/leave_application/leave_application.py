...
class LeaveApplication(Document):
    ...
    def update_attendance(self):
        ...
        for dt in daterange(getdate(self.from_date), getdate(self.to_date)):
            ...
            if self.create_or_update_attendance(attendance_name, date):
                success_count += 1

        if success_count > 0:
            frappe.msgprint(
                _("{0} attendance record(s) successfully created or updated with expected working hours and leave hours.").format(success_count),
                title="Success",
                indicator="green"
            )
    ...
    def create_or_update_attendance(self, attendance_name, date):
        ...
        try:
            ...
            frappe.log_error(f"Error processing attendance for {date}: {str(e)}")
            return False
