        ...
        for i, employee_name in enumerate(employees):
            ...
            if lark_user_info:
                ...
                try:
                    ...
                    r = requests.put('https://open.larksuite.com/open-apis/attendance/v1/user_stats_views/check?employee_type=employee_id', headers={
                        'Authorization': 'Bearer ' + tenant_access_token,
                    }, json={
                    ...
                    for day in r.get('data').get('user_data'):
                        date = None
                        working_hours = None
                        expected_hours = None
                        overtime = None
                    ...
                    try:
                        ...
                        if employee_shift:
                            ...
                            if shift_type.get('enable_attendance_calculation'):
                                ...
                                attendance.check_in_time_pairs = json.dumps(checkin_time_pairs)
...
def dispatch_calculation(calculation):
    return frappe.get_doc("AttendanceCalculation", calculation).dispatch()

def start_calculation(calculation):
    frappe.get_doc("AttendanceCalculation", calculation).start()
    ...
    for field, value in kwargs.items():
        if value:
            if isinstance(value, list):
                if len(value):
                    conditions.append("{0} IN ({})".format(field, ", ".join(["%s"] * len(value))))
                    values.extend([val.get('value') for val in value])
            else:
