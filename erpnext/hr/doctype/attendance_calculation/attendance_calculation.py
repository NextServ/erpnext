                        ...
                        try:
                            for data in day.get('datas'):
                                if data.get('code') == '51201':
                                    ...
                                    frappe.msgprint(f"Raw date: {date}")
                                    ...
                                    for feature in data.get('features'):
                                        ...
                                        if feature.get('key') == 'ShiftTime' and feature.get('value') != '-':
                                            ...
                                            if shift_in:
                                                ...
                                                frappe.msgprint(f"Parsed shift_in: {shift_in}")
                                ...
                                if data.get('code') == '51502-1-1' and data.get('value') != '-':
                                    ...
                                    if time_in:
                                        ...
                                        frappe.msgprint(f"Parsed time_in: {time_in}")
                                if data.get('code') == '51502-1-2' and data.get('value') != '-':
                                    ...
                                    if time_out:
                                        ...
                                        frappe.msgprint(f"Parsed time_out: {time_out}")
                            ...
                            if date:
                                try:
                                    ...
                                    parse(date)
                                    frappe.msgprint(f"Formatted date: {date}")
                                except Exception as e:
                                    frappe.msgprint(f"Date parsing error: {e}, Raw date: {date}")
                                    date = None
                            ...
                            if time_in and shift_in and time_in > shift_in:
                                calculated_late_in = (time_in - shift_in).total_seconds() / 3600
                                frappe.msgprint(f"Calculated late_in: {calculated_late_in} hours")
                                attendance.late_entry = True
                                attendance.late_in = calculated_late_in
                            elif duration_late_in > 0 and (not time_in or not shift_in or time_in <= shift_in):
                                frappe.msgprint(f"Using Lark duration_late_in: {duration_late_in} hours")
                                attendance.late_entry = True
